...
def boolean_standard_sequence(source_emission):
...
def boolean_variation_sequence(source_emission, tollerance_var):
...
def boolean_normalized_sequence(source_emission, tollerance_norm):
    sentiment = delta_emission(matrix.extract(source_emission))
    print(sentiment)
...
def main():
   ...
   # source = "D:\Dropbox\Git_Projects\Brexit-marketValue\datasets\Market_values.txt"
    ...
    # model = Hmm(T, O, I)
    # print("Filtering:")
    # print(model.filtering(19, boolean_standard_sequence(source_emission)))

    # if you want to use sentiment variation:
    T = matrix.build_transition_m(matrix.extract(source_ext), tollerance)
    O = matrix.build_emission_m(matrix.delta(matrix.extract(source), tollerance),
                                boolean_variation_sequence(source_emission, tollerance_var))
    I = [0.33, 0.33, 0.34]
    model = Hmm(T, O, I)
    print("Filtering:")
    print(model.filtering(19, boolean_variation_sequence(source_emission, tollerance_var)))
    ...
    # model = Hmm(T, O, I)
    # print("Filtering:")
    # print(model.filtering(19, boolean_normalized_sequence(source_emission, tollerance_norm)))
